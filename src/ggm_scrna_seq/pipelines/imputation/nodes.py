...
def select_gene_columns(df_counts: pd.DataFrame, columns_to_remove: Iterable) -> pd.DataFrame:
    log.info(f"Removing non-gene count columns {columns_to_remove} from total of {df_counts.shape[1]}")
...
def drop_genes_without_expression(df_counts: pd.DataFrame) -> pd.DataFrame:
    ...
    log.info(f"Removing {df_counts.shape[1] - len(genes_with_some_counts)} genes with no counts")
